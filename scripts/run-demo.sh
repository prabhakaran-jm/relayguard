...
if [[ "$DB_TARGET" == "cloud" ]]; then
  ...
  INCIDENT_ID="$(python -m apps.cli.create_incident --apply-schema --title "${RELAYGUARD_DEMO_TITLE:-API latency spike in us-east-1, primary health checks failing}" | tail -1)"
